describe('Validações de Alertas em JavaScript', () => {
    ...
    it('Deve confirmar um dialógo e validar a resposta positiva', () => {
    ...
    it('Deve cancelar um dialógo e validar a resposta positiva', () => {
    ...
    it('Deve interagir com um prompt, inserir um texto e validar uma mensagem', () => {
        cy.window().then((win) => {
            cy.stub(win, 'prompt').returns('Ford')
        })

        cy.on('window:alert', (msg) => {
            expect(msg).to.equal('Olá Ford! Boas-vindas ao WebDojo!')
        })
